...
In autoregressive decoder-only LLMs, the attention mechanism computes all-pair dot products between a query vector and all cached key vectors. At decoding step t, this requires O(t·d) MAD operations, where d is the head dimension. As sequence length grows, attention score computation dominates total inference time on CPUs: while MLPs, skip connections, and normalization remain O(1) per step, attention grows linearly with context length and quickly becomes the bottleneck [https://arxiv.org/html/2403.01273v1].

CPUs are poorly suited to the repetitive, highly parallel MAD workloads that GPUs handle efficiently. Prior CPU optimizations (e.g., weight quantization, sparse attention) either require training from scratch or do not address the MAD bottleneck directly [https://arxiv.org/html/2403.01273v1].

### 1.2 The Opportunity: SIMD In-Register Lookups

Modern CPUs provide SIMD registers (128-bit to 512-bit) that support single-instruction, multiple-data parallelism. Crucially, data stored in registers can be accessed in 1–2 CPU cycles, versus tens to hundreds of cycles for L1/L2 cache or main memory. NoMAD-Attention exploits this by shifting the computation paradigm from arithmetic (MAD) to memory lookups performed entirely within registers [https://arxiv.org/html/2403.01273v1].

---

## 2. How NoMAD-Attention Works

NoMAD-Attention introduces three algorithmic innovations to make register-resident lookup-based attention feasible [https://arxiv.org/html/2403.01273v1].

### 2.1 Product Quantization of Key Vectors

NoMAD adapts Product Quantization (PQ), originally designed for nearest-neighbor search [Jegou et al., 2010, https://ieeexplore.ieee.org/document/5432202], to estimate query-key dot products via table lookups.
...
This replaces the O(d) MADs per dot product with O(S) table lookups and additions [https://arxiv.org/html/2403.01273v1].

### 2.2 Compressing Lookup Tables into SIMD Registers

A naive PQ approach with 256 centroids (8-bit codes) and FP32 LUT entries requires 8192 bits per sub-quantizer—far exceeding the 128-bit width of universal SIMD registers (ARM NEON, baseline AVX). NoMAD solves this with two constraints [https://arxiv.org/html/2403.01273v1]:

1. **Constrained codebook size**: Each sub-quantizer uses exactly 16 centroids, yielding 4-bit codes. The motivation is empirical: attention outputs are known to lose rank extremely quickly [Dong et al., 2021, https://proceedings.mlr.press/v139/dong21a.html], suggesting key embeddings exhibit clustering amenable to small codebooks.
...
The quantization and de-quantization overhead is minimal because it is performed once per query per sub-quantizer, amortized over the entire context length [https://arxiv.org/html/2403.01273v1].

### 2.3 Memory Layout Reorganization for Batch Parallel Lookups

To maximize throughput, NoMAD reorganizes the key cache into a **transposed, blocked key-code cache** [https://arxiv.org/html/2403.01273v1]:
...
After processing all S sub-quantizers, the accumulated values are de-quantized, scaled by 1/√d, and passed through softmax [https://arxiv.org/html/2403.01273v1].
...
NoMAD requires per-layer, per-head codebooks because the value distributions of key embeddings vary significantly across layers and heads (verified empirically on LLaMA-2-7B) [https://arxiv.org/html/2403.01273v1].

The learning procedure is [https://github.com/tonyzhang617/nomad-dist]:
...
For a 32-layer, 32-head model (e.g., CodeLLaMA-7B), this produces 1024 separate codebooks [https://github.com/tonyzhang617/nomad-dist, local clone at /mnt/data/tzj/Code/nomad-dist].

### 3.2 Software and Hardware

- **Implementation**: Built in C/C++ on top of llama.cpp and FAISS [https://arxiv.org/html/2403.01273v1].
- **Distribution**: The repository provides pre-compiled binaries (`main`, `perplexity`, `quantize`) and Python scripts for codebook learning. The C++ source is withheld pending a patent application [https://github.com/tonyzhang617/nomad-dist].
- **Hardware**: Requires a CPU with AVX2 support. Experiments used a server with 2× Intel Xeon E5-2695 V3 (14 cores each) and 512 GB DDR4 RAM [https://arxiv.org/html/2403.01273v1].
...
NoMAD-Attention maintains model quality at aggressive compression ratios [https://arxiv.org/html/2403.01273v1]:
...
- **PCA-Attention**, a dimensionality-reduction baseline, fails catastrophically even at 2× compression. The authors attribute this to PCA’s symmetric dot-product computation versus NoMAD’s asymmetric computation (full-precision query, quantized keys) [https://arxiv.org/html/2403.01273v1].
- Beyond 8× compression (d_sub > 1), quality degrades, but NoMAD still outperforms PCA.

### 4.2 Inference Efficiency

End-to-end speedups are measured on CodeLlama-7B (4-bit and 16-bit weights) generating 4096 tokens after prompts up to 16k tokens [https://arxiv.org/html/2403.01273v1]:
...
The speedup grows with context length because attention dominates runtime more strongly as the KV cache grows [https://arxiv.org/html/2403.01273v1].

### 4.3 Ablation: Why In-Register Matters

An ablation comparing MAD-based Attention, PQ-Attention (8-bit codes, d_sub=2), and NoMAD-Attention (4-bit codes, d_sub=1) at 16k queries / 16k context on a single thread reveals [https://arxiv.org/html/2403.01273v1]:
...
- **Academic**: Accepted at NeurIPS 2024. Available through arXiv, OpenReview, and ML Anthology [https://proceedings.neurips.cc/paper_files/paper/2024/hash/ccda3c632cc8590ee60ca5ba226a4c30-Abstract-Conference.html, https://openreview.net/forum?id=4xDxVQHsbZ, https://mlanthology.org/neurips/2024/zhang2024neurips-nomadattention/].
- **Open-source integration**: As of the research date, NoMAD-Attention has **not been upstreamed** into llama.cpp or other major inference frameworks. A llama.cpp issue (#7532, May 2024) raised the question of adoption, but no merge or implementation is evident [https://github.com/ggerganov/llama.cpp/issues/7532].
- **Binary-only distribution**: The official repository (tonyzhang617/nomad-dist) provides binaries and pre-computed codebooks but not the core C++ source, limiting independent verification and portability [https://github.com/tonyzhang617/nomad-dist].

### 5.2 Related Subsequent Work

- **EcoTransformer** (arXiv:2507.20096) also explores attention without multiplication, but uses an L1-distance-based Laplacian kernel rather than PQ+SIMD lookups. It is architecturally distinct from NoMAD [https://www.arxiv.org/pdf/2507.20096].
...
1. Zhang, T., Yi, J. W., Yao, B., Xu, Z., & Shrivastava, A. (2024). *NoMAD-Attention: Efficient LLM Inference on CPUs Through Multiply-add-free Attention*. arXiv:2403.01273. https://arxiv.org/abs/2403.01273
2. Zhang et al. (2024) — Full HTML text. https://arxiv.org/html/2403.01273v1
3. NeurIPS 2024 Proceedings Abstract. https://proceedings.neurips.cc/paper_files/paper/2024/hash/ccda3c632cc8590ee60ca5ba226a4c30-Abstract-Conference.html
4. OpenReview forum. https://openreview.net/forum?id=4xDxVQHsbZ
5. ML Anthology entry. https://mlanthology.org/neurips/2024/zhang2024neurips-nomadattention/
6. Official distribution repository (README, binaries, codebook learning script). https://github.com/tonyzhang617/nomad-dist (local clone verified at /mnt/data/tzj/Code/nomad-dist)
7. Jegou, H., Douze, M., & Schmid, C. (2010). Product quantization for nearest neighbor search. *IEEE TPAMI*, 33(1), 117–128. https://ieeexplore.ieee.org/document/5432202
8. Dong, Y., Cordonnier, J.-B., & Loukas, A. (2021). Attention is not all you need: Pure attention loses rank doubly exponentially with depth. *ICML 2021*. https://proceedings.mlr.press/v139/dong21a.html
9. llama.cpp GitHub Issue #7532 — Discussion of NoMAD-Attention adoption. https://github.com/ggerganov/llama.cpp/issues/7532
10. EcoTransformer (arXiv:2507.20096) — Related subsequent work on attention without multiplication. https://www.arxiv.org/pdf/2507.20096
